...
cmds[kind-with-metallb]='kind create cluster && helm repo add bitnami https://charts.bitnami.com/bitnami && helm repo up && helm upgrade --install -n metallb --create-namespace --set configInline.address-pools[0].name=default --set configInline.address-pools[0].protocol=layer2 --set configInline.address-pools[0].addresses={"$(docker network inspect kind -f "{{(index .IPAM.Config 0).Subnet}}"|cut -d"." -f1-2).255.1-$(docker network inspect kind -f "{{(index .IPAM.Config 0).Subnet}}"|cut -d"." -f1-2).255.250"} metallb bitnami/metallb && ([ "${1:-}" != "--with-ingress-nginx" -a "${2:-}" != "--with-ingress-nginx" ] || (helm repo add ingress-nginx https://kubernetes.github.io/ingress-nginx && helm upgrade --install ingress-nginx ingress-nginx/ingress-nginx -n ingress-nginx --create-namespace))'
